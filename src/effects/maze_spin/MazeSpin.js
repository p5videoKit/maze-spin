...
export default class MazeSpin {
  ...
  prepareOutput() {
    ...
    // console.log('MazeSpin prepareOutput input.canvas', this.input.canvas);
    // console.log('MazeSpin prepareOutput input_copy', this.input_copy);
    // console.log('MazeSpin prepareOutput input_copy.copy', this.input_copy.copy);
    // if (!this.input_copy) {
    //   console.log('MazeSpin prepareOutput no input_copy');
    //   return;
    // }
    // if (!this.input_copy.copy) {
    //   console.log('MazeSpin prepareOutput no input_copy.copy');
    //   return;
    // }
    if (!this.input) {
      console.log('MazeSpin prepareOutput no input');
      return;
    }
    if (this.video_color) {
      // copy(srcImage, sx, sy, sw, sh, dx, dy, dw, dh)
      // p5.js/1.11.1 needs srcImage.canvas on input
      // _main.default.prototype._copyHelper = function (dstImage, srcImage, sx, sy, sw, sh, dx, dy, dw, dh) {
      //   var s = srcImage.canvas.width / srcImage.width;
      this.input_copy = this.input.get();
